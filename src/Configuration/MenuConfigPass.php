...
use EasyCorp\Bundle\EasyAdminBundle\Event\PostMenuProcessEvent;
...
class MenuConfigPass implements ConfigPassInterface
{
    ...
    public function process(array $backendConfig): array
    {
        ...
        $event = new PostMenuProcessEvent($backendConfig);
        $this->eventDispatcher->dispatch($event);
        $backendConfig = $event->getMenuConfig();
    ...
    private function processMenuConfig(array $menuConfig, array $backendConfig, $parentItemIndex = -1): array
    {
        foreach ($menuConfig as $i => $itemConfig) {
