...
class ViewConfigPass implements ConfigPassInterface
{
    ...
    private function processViewConfig(array $backendConfig): array
    {
     ...
     */
    private function processDefaultFieldsConfig(array $backendConfig): array
    {
        foreach ($backendConfig['entities'] as $entityName => $entityConfig) {
            foreach (['edit', 'list', 'new', 'search', 'show'] as $view) {
     ...
     * the user experience.
     */
    private function processFieldConfig(array $backendConfig): array
    {
        foreach ($backendConfig['entities'] as $entityName => $entityConfig) {
            foreach (['edit', 'list', 'new', 'search', 'show'] as $view) {
     ...
     */
    private function processPageTitleConfig(array $backendConfig): array
    {
        foreach ($backendConfig['entities'] as $entityName => $entityConfig) {
            foreach (['edit', 'list', 'new', 'search', 'show'] as $view) {
     ...
     * overridden individually by each entity.
     */
    private function processMaxResultsConfig(array $backendConfig): array
    {
        foreach ($backendConfig['entities'] as $entityName => $entityConfig) {
     ...
     */
    private function processSortingConfig(array $backendConfig): array
    {
        foreach ($backendConfig['entities'] as $entityName => $entityConfig) {
     ...
     */
    private function getFieldFormat($fieldType, array $backendConfig): ?string
    {
