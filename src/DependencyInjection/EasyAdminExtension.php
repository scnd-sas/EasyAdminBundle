...
class EasyAdminExtension extends Extension
{
     ...
     */
    public function processConfigFiles(array $configs): array
    {
        $existingEntityNames = [];
    ...
    private function normalizeEntityConfig($entityName, $entityConfig)
    {
        ...
        if (!isset($entityConfig['class']) && !isset($entityConfig['dto_class']) && !isset($entityConfig['search_class'])) {
            throw new RuntimeException(sprintf('The "%s" entity must define its associated Doctrine entity class using the "class" option or the "dto_class" option.', $entityName));
        }
